...
namespace ValueTypes
{
    ...
    {
        /// <summary>
        /// Geben Sie die Fläche des Rechtecks (Länge x Breite) zurück.
        ...
        {
            decimal preis = (nettopreis*steuerProdukt) ?? (nettopreis*steuerKategorie) ?? (nettopreis*(decimal)1.2); 
            return preis;
        }
        ...
        public double BerechneSchuelerProKlasse(int schuelerGesamt, int klassenGesamt)
        {
            ...
            if (klassenGesamt > 0)
            {
                anzahl = 1.00 * schuelerGesamt / klassenGesamt;
            }
            return anzahl;

            //return (klassenGesamt > 0) ? (1.00 * schuelerGesamt / klassenGesamt) : 0;
